...
class NeuralNetwork {
  ...
  train(inputX: number, inputY: number, targetR: number, targetG: number, targetB: number): number {
    const numLayers = this.weights.length
    ...
    for (let l = 0; l < numLayers; l++) {
      ...
      for (let j = 0; j < outputSize; j++) {
        ...
        for (let k = 0; k < inputSize; k++) {
          const idx = offset + k
          const grad = delta[j] * prevActivation[k] - this.l2Decay * weights[idx]
          weightVel[idx] = this.momentum * weightVel[idx] + this.learningRate * grad
...
function trainingLoop() {
  ...
  if (lossHistory.length === LOSS_HISTORY_SIZE && learningRate > minLearningRate) {
